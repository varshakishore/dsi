...
class IndexingCollator(DataCollatorWithPadding):
    def __call__(self, features):
        ...
        docids = [id2label[x[1]] for x in features]
        # docids = np.random.randint(0, 5, len(docids))
...
def main():
    ...
    train_data = load_dataset_helper(os.path.join(args.base_data_dir, 'new_docs', 'trainqueries.json'))
    generated_queries = load_dataset_helper(os.path.join(args.base_data_dir, 'new_docs', 'passages_seen.json'))
    val_data = load_dataset_helper(os.path.join(args.base_data_dir, 'new_docs', 'valqueries.json'))
    ...
    global id2label
    # import pdb;pdb.set_trace()
    id2label = dict(zip(range(np.max(joblib.load(os.path.join(args.base_data_dir, 'new_docs', 'doc_list.pkl')))), np.random.randint(0, 4+1, np.max(joblib.load(os.path.join(args.base_data_dir, 'new_docs', 'doc_list.pkl'))))))
    class_num = 5
    if args.base_data_dir_new or args.test_only:
    ...
    if args.freeze_base_model:
        ...
        for name, param in model.named_parameters():
            if name !='projection.weight': #classifier.weight':
    ...
    doc_class = joblib.load(os.path.join(args.base_data_dir, 'new_docs', 'doc_class.pkl'))
